...
const buildLockPath = join(projectRoot, 'android', '.build-apk.lock');
...
if (isWindows) {
  ...
  run(process.env.ComSpec ?? 'cmd.exe', [
    ...
    '/s',
    '/c',
    `""${join(androidRoot, 'gradlew.bat')}" :app:assembleRelease --no-daemon -PreactNativeArchitectures=${architectures.join(',')}"`,
...
const sourceApk = join(androidRoot, 'app', 'build', 'outputs', 'apk', 'release', 'app-release.apk');
...
const architectureLabel = architectures.length === 1 ? `-${architectures[0]}` : '-universal';
const artifactPath = join(artifactDir, `${appName}-${version}${architectureLabel}-release.apk`);
